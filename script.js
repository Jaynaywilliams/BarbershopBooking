...
const BARBERS=[
  ...
  {id:'Calvin',name:'Calvin',initials:'Cal'},
  {id:'Dre',name:'Dre',initials:'Dre'},
  {id:'Mike',name:'Mike',initials:'Mike'},
];
...
$('shopPhone').href=`tel:${SHOP.phone.replace(/[^\d+]/g,'')}`; 
...
function toISOWithTZ(local){
  const dt=new Date(local);
  const off=dt.getTimezoneOffset();
  const ms=off*60*1000;
  const loc=new Date(dt.getTime()-ms);
  return loc.toISOString().replace('Z',offset(off));
}
...
form.addEventListener('submit',async e=>{
  ...
  // FIXED LINE (was slicing and breaking layout)
